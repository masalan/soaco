...
class Stations extends Main
{
    ...
    function create()
    {
        ...
        if ($this->input->server('REQUEST_METHOD')=='POST') {
            ...
            if (!isset($upload_data['error'])) {
                ...
                $city_data['phone'] = htmlentities($this->input->post('phone'));
                ...
                $city_data['twon_id'] = htmlentities($this->input->post('twon_id'));
                $city_data['city_id'] = $this->get_current_city()->id;
                ...
                //$this->db->insert('cd_stations',$city_data);


                   if (!$this->db->insert('cd_stations',$city_data)) {
                       // if (!$this->save_update(&$city_data)) {
                            $this->session->set_flashdata('error','Database error occured.Please contact your system administrator.');
                }
                foreach ($upload_data as $upload) {
                    $image = array(
                        'parent_id'=>$city_data['id'], //$this->user->get_logged_in_user_info()->user_id, //$city_data['id'],
                        'type' => 'station',
                ...
                $this->session->set_flashdata( 'success', 'Congratulation! Votre nouvelle Station a ete cree' );
                redirect(site_url('stations/create'));
            } else {
        ...
        $data['city_id'] = $this->get_current_city()->id;
        $content['content'] = $this->load->view('stations/create',$data,true);
        $this->load_template($content,false);
    }


    function save_update(&$data, $id=false)
    {
        //if there is no data with this id, create new
        if (!$id && !$this->exists(array('id'=>$id))) {
            if ($this->db->insert('cd_stations',$data)) {
            ...
            $this->db->where('id',$id);
            return $this->db->update('cd_stations',$data);
        }
    ...
    function email_to_admin()
    {
        ...
        $url = site_url();
        $subject = 'New Station are waiting for approval';
        $html = <<<EOT
<p>Hi</p>

<p>Good day.</p>

<p>New Station are waiting for approval.</p>
<a href='$url'>Go to Station DashBoard</a>
...
EOT;
    ...
    function edit( $city_id = 0 )
    {
        ...
        if ($this->input->server('REQUEST_METHOD')=='POST') {
            ...
            $city_data['lng'] = htmlentities($this->input->post('lng'));
             ...
             $this->db->update('cd_stations',$city_data);
            ...
            redirect(site_url('stations/edit/' . $city_id));
        }



        $data['city'] = $this->get_info($city_id);
    ...
    {
        $query = $this->db->get_where('cd_stations', array('id'=>$id));

        if ($query->num_rows() == 1) {
            return $query->row();
        } else {
            return $this->get_empty_object('cd_stations');
        }
    }
     ...
     * @param $cat_id
     */
    function get_com($cat_id)
    {
        $arr = $this->db->get_where('cd_arrondissement', array('cd_commune_id'=>$cat_id));
       // $arr = $this->db->get('cd_arrondissement');

        // $arr = $this->sub_category->get_all_by_cat_id($cat_id);
