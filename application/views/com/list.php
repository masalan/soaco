			...
			<div class='row'>
				<div class='col-sm-9'>
					...
					echo form_open(site_url('com/search'), $attributes);
					?>
				...
				<div class='col-sm-3'>
					<a href='<?php echo site_url('com/add');?>' class='btn btn-primary pull-right'><span class='glyphicon glyphicon-plus'></span>
				...
				?>
						<tr>
							...
							<?php 
								if(!$this->session->userdata('is_city_admin')) { 
									if(in_array('edit',$allowed_accesses)):?>
										<td><a href='<?php echo site_url("com/edit/".$feed->id);?>'><i class='glyphicon glyphicon-edit'></i></a></td>
							<?php endif; } else { ?>
										<td><a href='<?php echo site_url("com/edit/".$feed->id);?>'><i class='glyphicon glyphicon-edit'></i></a></td>
							<?php } ?>
							...
							<?php 
								if(!$this->session->userdata('is_city_admin')) { 
									if(in_array('delete',$allowed_accesses)):?>
									<td><a href='<?php echo site_url("com/delete/".$feed->id);?>'><i class='glyphicon glyphicon-trash'></i></a></td>
							<?php endif; } else {?>
									<td><a href='<?php echo site_url("com/delete/".$feed->id);?>'><i class='glyphicon glyphicon-trash'></i></a></td>
							<?php } ?>
										...
										<td>
											<?php if($feed->is_published == 1):?>
												...
												<button class="btn btn-sm btn-primary unpublish"   
													feedId='<?php echo $feed->id;?>'>Oui
												</button>
												
											<?php else:?>
											
												<button class="btn btn-sm btn-danger publish"
												feedId='<?php echo $feed->id;?>'>Non</button>
			...
			$(document).ready(function(){
				$(document).delegate('.publish','click',function(){
					...
					$.ajax({
						url: '<?php echo site_url('com/publish');?>/'+id,
						method:'GET',
				...
				$(document).delegate('.unpublish','click',function(){
					...
					$.ajax({
						url: '<?php echo site_url('com/unpublish');?>/'+id,
						method:'GET',
						success:function(msg){
							if(msg == 'true')
